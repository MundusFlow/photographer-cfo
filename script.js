...
const RUSH_SURCHARGE_RATE = 0.25;
...
// ---------------------------------------------------------------------------
const UPGRADE_HTML =
  'Want this saved automatically for every session, with a full profit dashboard? ' +
  '<a href="#cta">Get The Photographer CFO &rarr;</a>';
...
(() => {
  ...
  const rushInput = document.getElementById("q_emergency");
  ...
  wireClearButton(
    "q_clearBtn",
    [materialsInput, markupInput, hoursInput, rateInput, permitsInput, rushInput, marginInput],
    btn,
    resultEl
  );
  ...
  function calculate() {
    ...
    const isRush = rushInput.checked;
    ...
    const rushAmount = isRush ? preSurcharge * RUSH_SURCHARGE_RATE : 0;
    const subtotal = preSurcharge + rushAmount;
    const profitBuffer = subtotal * (marginPct / 100);
    const quoteTotal = subtotal + profitBuffer;

    const interpretation = `Charging ${formatMoney(quoteTotal)} covers your ${formatMoney(subtotal)} in gear, labor, travel${isRush ? ", and the rush surcharge" : ""}, and still bags you ${formatMoney(profitBuffer)} in profit &mdash; right at your ${formatPercent(marginPct)} target margin.`;
    const benchmark = "Target margins for photography sessions commonly run 30-40% (general estimate).";
    ...
    resultEl.innerHTML = `
      <div class="result-box">
        ...
        <div class="result-breakdown">
          ...
          <div class="breakdown-row"><span>Gear &amp; products (+${formatPercent(markupPct)} markup)</span><span>${formatMoney(materialsWithMarkup)}</span></div>
          <div class="breakdown-row"><span>Shoot + editing (${hours} hrs &times; ${formatMoney(rate)})</span><span>${formatMoney(laborCost)}</span></div>
          <div class="breakdown-row"><span>Travel / location fees</span><span>${formatMoney(permits)}</span></div>
          ${isRush ? `<div class="breakdown-row"><span>Rush surcharge (${formatPercent(RUSH_SURCHARGE_RATE * 100)})</span><span>${formatMoney(rushAmount)}</span></div>` : ""}
        ...
        ${followUpBlock("#session-profit", "Session Profit Calculator")}
      </div>
    `;
  }
...
(() => {
  ...
  function calculate() {
    ...
    const interpretation = profit >= 0
      ? `You collected ${formatMoney(invoiced)} and kept ${formatMoney(profit)} of it &mdash; a ${formatPercent(margin)} margin${isLowMargin ? ", thinner than it should be for the time you put in" : ", a healthy return for this session"}.`
      : `You collected ${formatMoney(invoiced)} but actually lost ${formatMoney(Math.abs(profit))} once every cost is counted &mdash; this session cost you money to do.`;
    const benchmark = "Target margins for photography sessions commonly run 30-40% (general estimate).";

    resultEl.innerHTML = `
      <div class="result-box">
        <p class="result-headline-label">Session Profit</p>
        ...
        <div class="result-breakdown">
          ...
          <div class="breakdown-row"><span>&minus; Gear &amp; products</span><span>${formatMoney(materials)}</span></div>
          <div class="breakdown-row"><span>&minus; Shoot + editing labor</span><span>${formatMoney(labor)}</span></div>
          <div class="breakdown-row"><span>&minus; Other session costs</span><span>${formatMoney(other)}</span></div>
        ...
        <p class="result-tip ${isLowMargin ? "warning-tip" : ""}">
          ${isLowMargin
            ? "&#9888;&#65039; Under 15% margin&mdash;this session barely covered overhead. Check your next quote."
...
(() => {
  ...
  const gearInput = document.getElementById("hr_gear");
  const insuranceInput = document.getElementById("hr_insurance");
  const softwareInput = document.getElementById("hr_software");
  const marketingInput = document.getElementById("hr_marketing");
  const studioInput = document.getElementById("hr_studio");
  ...
  function calculate() {
    ...
    const monthlyOverhead =
      num(gearInput) + num(insuranceInput) + num(softwareInput) + num(marketingInput) + num(studioInput) + num(otherOverheadInput);
    ...
    const benchmark = "Independent photographer rates commonly range $75-$250/hr of shoot + editing time depending on niche and region (rough estimate).";
    ...
    resultEl.innerHTML = `
      <div class="result-box">
        ...
        <div class="result-breakdown">
          ...
          <div class="breakdown-row"><span>&minus; Gear / product COGS</span><span>${formatMoney(materials)}</span></div>
  ...
  function calculate() {
    ...
    resultEl.innerHTML = `
      <div class="result-box">
        ...
        ${followUpBlock("#quote-calculator", "Quote Calculator")}
      </div>
    `;
  }
